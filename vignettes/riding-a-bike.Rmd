...
- **Max distance:** `r round(max(st_length(sf)))`
  
- **Min distance:**  `r round(min(st_length(sf)), 2)`
  
- **Average distance:** `r round(mean(st_length(sf)))`
...
av::av_capture_graphics(make_plot(), video_file, 512 , 480, vfilter = "framerate=fps=4")

```

<img alt="Trips Animation" href="https://github.com/fozy81/opendatascot/blob/master/vignettes/images/output.gif?raw=true" width="350" align="center"/>
...
NOTE: This requires having [Open Source Routing Machine](http://project-osrm.org/) (OSRM) library to be installed locally. Please follow link and install. Additionally, the OpenStreetMap data for Scotland needs to be downloaded and add to OSRM.
...
The resulting animations shows the shortest routes cyclists may use to travel A to B using the cycle network. Of course, people hiring bikes may have chosen a different route but it gives some idea how the transport network in Glasgow looks those hiring bikes. 

<img alt="Routes Animation" href="https://github.com/fozy81/opendatascot/blob/master/vignettes/images/routes.gif?raw=true" width="350" align="center"/>
